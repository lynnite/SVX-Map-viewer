document.addEventListener("DOMContentLoaded", () => {
  ...
  labelsLayer.id = "map-labels-layer";
  ...
  z-index: 6;
  `;
  mapContainer.appendChild(labelsLayer);
  ...
  function applyLabelsVisibility() {
    ...
    if (toggleLabelsBtn) toggleLabelsBtn.classList.toggle("active", labelsVisible);
  }
  ...
  if (toggleLabelsBtn) {
    toggleLabelsBtn.addEventListener("click", () => {
      ...
      localStorage.setItem("labelsVisible", labelsVisible ? "true" : "false");
      applyLabelsVisibility();
    });
  }
  ...
  function renderMapLabels(imgSrc) {
    ...
    const labels = window.MAP_LABELS.filter(
      lbl => !lbl.map || lbl.map === fileName || lbl.map === imgSrc
    );
    ...
    labels.forEach(lbl => {
      ...
      let textShadow = "none";
      if (strokeWidth > 0) {
        const c = lbl.strokeColor || "#000000";
        const steps = 12;
        const shadows = [];
        for (let i = 0; i < steps; i++) {
          const angle = (i / steps) * 2 * Math.PI;
          const dx = (Math.cos(angle) * strokeWidth).toFixed(2);
          const dy = (Math.sin(angle) * strokeWidth).toFixed(2);
          shadows.push(`${dx}px ${dy}px 0 ${c}`);
        }
        textShadow = shadows.join(", ");
      }

      const pillStyle = lbl.pillOn
      ? `background-color: ${hexToRgba(lbl.pillColor, lbl.pillOpacity)}; padding: 0.15em 0.5em; border-radius: 4px;`
      : "";
      ...
      font-family: ${lbl.fontFamily || "Arial"}, sans-serif;
      font-size: ${lbl.fontSize || 40}px;
      font-weight: ${lbl.bold ? "bold" : "normal"};
      font-style: ${lbl.italic ? "italic" : "normal"};
      letter-spacing: ${lbl.letterSpacing || 0}px;
      color: ${lbl.color || "#ffffff"};
  ...
  mapContainer.addEventListener("panzoomchange", () => {
    ...
    window.addEventListener("pointerdown", (e) => {
      ...
      function entityMatchesQuery(e, query) {
        ...
        const haystack = (e.proto + " " + (typeof getEntityDisplayName === "function" ? getEntityDisplayName(e.proto) : e.proto)).toLowerCase();
      ...
      function executeSearch(queryOverride) {
        ...
        if (currentSearchResults.length > 0) {
          ...
          if (resolvedProto) {
            searchInput.value = typeof getEntityDisplayName === "function" ? getEntityDisplayName(resolvedProto) : resolvedProto;
          }
        } else {
          currentSearchResults = [];
          currentSearchIndex = 0;
      ...
      function pickBestMatchingProto(matches, query) {
        ...
        for (const e of matches) {
          if (e.proto && !seen.has(e.proto)) {
            const displayName = typeof getEntityDisplayName === "function" ? getEntityDisplayName(e.proto) : e.proto;
            seen.set(e.proto, displayName.toLowerCase());
          }
        }
      ...
      searchInput.addEventListener("input", () => {
        ...
        if (matches.length > 0) {
          ...
          matches.forEach(({ entity: match, count }) => {
            ...
            const displayName = typeof getEntityDisplayName === "function" ? getEntityDisplayName(match.proto) : match.proto;
      ...
      async function loadInsertMetadata(mapFolderName) {
        ...
        try {
          ...
          const parsedYaml = window.jsyaml ? jsyaml.load(rawText) : null;
      ...
      function renderEntityMenuItem(item, isInsert) {
        ...
        const itemName = typeof getEntityDisplayName === "function" ? getEntityDisplayName(item.proto) : item.proto;
        ...
        if (item.contents && item.contents.length > 0) {
          item.contents.forEach(child => {
            ...
            const childName = typeof getEntityDisplayName === "function" ? getEntityDisplayName(child.proto) : child.proto;
